...
async def subscription_callback(update: Update, context: CallbackContext):
    ...
    if await check_subscription(user_id, context):
        ...
        if is_admin(user_id):
            await query.edit_message_text("✅ <b>Siz barcha kanallarga muvaffaqiyatli obuna bo'ldingiz!</b>\n\n📱 Botdan foydalanishni boshlang.", parse_mode="HTML", reply_markup=get_admin_keyboard())
        else:
            await query.edit_message_text("✅ <b>Siz barcha kanallarga muvaffaqiyatli obuna bo'ldingiz!</b>\n\n📱 Botdan foydalanishni boshlang.", parse_mode="HTML", reply_markup=get_main_keyboard())
    else:
        keyboard = await get_subscription_keyboard(user_id, context)
        if keyboard is None:
            db_user = get_user(user_id)
...
async def referral_callback_handler(update: Update, context: CallbackContext):
    ...
    if data == "copy_referral_link":
        referral_link = f"https://t.me/{BOT_USERNAME}?start={user_id}"
        await query.edit_message_text(f"📋 <b>Referral link</b>\n\n<code>{referral_link}</code>\n\n📤 Linkni nusxalab, do'stlaringizga ulashing!", parse_mode="HTML", reply_markup=get_referral_link_keyboard())
...
async def promo_callback_handler(update: Update, context: CallbackContext):
    ...
    if data.startswith("promo_"):
        ...
        if user_coins < coins_needed:
            await query.edit_message_text(f"❌ Coinlaringiz yetarli emas!\n\n💰 Sizda: {user_coins} coin\n💰 Kerak: {coins_needed} coin\n💰 Yetishmayapti: {coins_needed - user_coins} coin", reply_markup=get_promo_keyboard())
            return
        ...
        await query.edit_message_text(f"✅ <b>Promo muvaffaqiyatli almashtirildi!</b>\n\n📌 Promo: {promo_name}\n💰 Sarflangan coin: {coins_needed}\n💎 Qolgan coin: {new_coins}\n\n🎫 <b>Promo kodingiz:</b>\n<code>{promo_code}</code>\n\n📝 Promo kodni saqlab qo'ying!", parse_mode="HTML", reply_markup=get_promo_keyboard())
...
async def admin_coin_callback_handler(update: Update, context: CallbackContext):
    ...
    if data in coin_map:
        ...
        await query.edit_message_text(f"💰 {coin_map[data]} coin {'berish' if coin_map[data] > 0 else 'olish'}\n\nFoydalanuvchi ID sini yozing:\n<code>/coin ID</code>\n\nMisol: <code>/coin 123456789</code>", parse_mode="HTML", reply_markup=get_admin_coin_keyboard())
...
async def admin_coin_command_handler(update: Update, context: CallbackContext):
    try:
        ...
        args = context.args
        if len(args) < 2:
            await update.message.reply_text("❌ Noto'g'ri format!\n<code>/coin 123456789 50</code> - coin berish\n<code>/coin 123456789 -50</code> - coin olish", parse_mode="HTML")
            return
        ...
        try:
            await context.bot.send_message(chat_id=target_id, text=f"{'✅' if amount > 0 else '❌'} Admin tomonidan {'berildi' if amount > 0 else 'olindi'}: <b>{abs(amount)} coin</b>\n💰 Joriy coin: <b>{new_coins}</b>", parse_mode="HTML")
        except:
            pass
        await update.message.reply_text(f"✅ {target_id} ID li foydalanuvchiga {amount} coin {'berildi' if amount > 0 else 'olindi'}!\n💰 Joriy coin: {new_coins}", reply_markup=get_admin_keyboard())
    except Exception as e:
...
async def admin_broadcast_handler(update: Update, context: CallbackContext):
    try:
        ...
        await update.message.reply_text("📢 <b>Xabar yuborish</b>\n\nYubormoqchi bo'lgan xabaringizni yozing:\n💡 <i>Barcha foydalanuvchilarga yuboriladi</i>\n\n❌ Bekor qilish uchun /cancel deb yozing", parse_mode="HTML", reply_markup=get_admin_keyboard())
    except Exception as e:
...
async def admin_broadcast_send(update: Update, context: CallbackContext):
    try:
        ...
        await progress_msg.edit_text(f"✅ Xabar yuborildi!\n\n📤 Yuborildi: {sent}\n❌ Yuborilmadi: {failed}\n👥 Jami: {len(users)}", reply_markup=get_admin_keyboard())
        context.user_data['broadcast'] = False
    except Exception as e:
